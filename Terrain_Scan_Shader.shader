Shader "GameDevBuddies/Terrain_Scan_Shader"
{
    Properties
    {
        ...
        _Terrain_Scan_Direction_XZ("Terrain Scan Direction (XZ)", Vector) = (1.0, 0.0, 0, 0)
        _Terrain_Scan_Range("Terrain Scan Range", Range(0.1, 2000)) = 100
        _Terrain_Scan_Arc_Angle("Terrain Scan Arc Angle", Range(0, 180)) = 150        

        [Header(Edge Gradient)] [Space]
        _Edge_Gradient_Start_Distance_Percentage("Edge Gradient Start Distance (Percentage)", Range(0.0, 1.0)) = 0.85
        _Edge_Gradient_Falloff("Edge Gradient Falloff", Range(1.0, 10.0)) = 2.5
        ...
        _Distance_Between_Scan_Lines("Distance Between Scan Lines", Range(0.5, 10)) = 1
        _Scan_Lines_Min_Visible_Distance("Scan Lines Min Visible Distance", Range(0.0, 10.0)) = 3.0
        _Scan_Lines_Visibility_Falloff("Scan Lines Visibility Falloff", Range(0.0, 10.0)) = 0.35
        [HDR]_Scan_Line_Color("Scan Line Color", Color) = (2.389936, 4.074151, 8, 0)

        [Header(Ground Darken)] [Space]
        _Ground_Darken_Edge_Offset("Ground Darken Edge Offset", Range(0, 1)) = 0.2
        _Ground_Darken_Edge_Falloff("Ground Darken Edge Falloff", Range(0.000001, 2.0)) = 0.475 
        _Ground_Darken_Color("Ground Darken Color", Color) = (0, 0, 0, 0)

        [Header(Ground Darken Circle)] [Space]
        _Dark_Circle_Size("Dark Circle Size", Range(0, 10)) = 1.5
        ...
        _Side_Edge_Soften_Arc_Angle("Side Edges Soften Arc Angle", Range(0, 180)) = 145
        _Side_Edge_Soften_Falloff("Side Edges Soften Falloff", Range(1.0, 10.0)) = 6.0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float SobelSampleDepth(float2 uv, float2 pixelOffset)
            {
                ...
                float2 verticalOffset = float2(pixelOffset.y, 0);
            ...
            float3 TerrainScanFragmentFunction(SurfaceDescriptionInputs IN){
                ...
                // -- Darkening effect. -- 
                float darkeningEffect = 1.0 - pow(1.0 - saturate(distanceFromOrigin01 + _Ground_Darken_Edge_Offset), _Ground_Darken_Edge_Falloff) / pow(saturate(1.0 - _Ground_Darken_Edge_Offset), _Ground_Darken_Edge_Falloff);
                float3 darkenedOriginalColor = Blend_Darken(originalColor, _Ground_Darken_Color.rgb, angleMask * distanceFromOriginMask * darkeningEffect * _Ground_Darken_Color.a);
                ...
                float sobelOutlineMask = saturate(SobelSampleDepth(screenPosition, pixelOffset));
